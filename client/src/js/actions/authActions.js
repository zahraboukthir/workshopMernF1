...
export const register = (newuser, navigate) => async (dispatch) => {
  ...
  try {
    const res = await axios.post("http://localhost:7000/auth/signup", newuser);
...
export const loginuser = (user, navigate) => async (dispatch) => {
  ...
  try {
    const res = await axios.post("http://localhost:7000/auth/signin", user);
    dispatch({ type: LOGIN_SUCCESS, payload: res.data });
    navigate(
      res.data.user.role == "client"
        ? "/dhbClient"
        : res.data.user.role == "saler"
        ? "/dhbSaler"
        : "/"
    );
  } catch (error) {
